...
const OrdersModal = (props) => {
  ...
  const deleteProductFromOrderUrl = `http://localhost:3000/deleteProduct/${orderId}/${productId}`;
  ...
  const deleteProductFromOrder = async () => {
    try {
      const response = await fetch(deleteProductFromOrderUrl, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
          ...
          orderArray.map((e) => (
            ...
            >
              ...
              <Button
                sx={{ marginBottom: 5 }}
                onClick={() => {
                  dispatch(loginSlice.actions.setproductId(e.product_id)),
                    deleteProductFromOrder().then(orderDetails());
                  console.log("deneme calisiyor " + productId);
                }}
              >
